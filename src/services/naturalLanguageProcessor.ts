...
export class NaturalLanguageProcessor {
  ...
  private async analyzeRequest(nlRequest: NaturalLanguageRequest) {
    // Build context from conversation history
    const context = nlRequest.conversationHistory && nlRequest.conversationHistory.length > 0 
      ? `Previous conversation:\n${nlRequest.conversationHistory.map(item => 
          `User: ${item.request}\nResult: ${item.response.success ? 'Success' : 'Error'}`
        ).join('\n')}\n\n`
      : '';
...
Analyze this request and provide the structured JSON response.`;
  ...
  private fallbackAnalysis(request: string) {
    const lowerRequest = request.toLowerCase();
    
    // Determine objects mentioned
    const objects = [];
    if (lowerRequest.includes('account')) objects.push('Account');
    if (lowerRequest.includes('contact')) objects.push('Contact');
    if (lowerRequest.includes('lead')) objects.push('Lead');
    if (lowerRequest.includes('opportunity') || lowerRequest.includes('opp')) objects.push('Opportunity');
    if (lowerRequest.includes('case')) objects.push('Case');
