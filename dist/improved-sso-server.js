...
import { createSessionSalesforceConnection, SalesforceConnectionError } from "./utils/improvedConnection.js";
...
async function handleToolCall(name, args, conn) {
    switch (name) {
        ...
        case "salesforce_manage_object": {
            const manageObjectArgs = args;
            if (!manageObjectArgs.operation) {
                throw new Error('operation is required for manage object');
            }
            const validatedArgs = {
                operation: manageObjectArgs.operation,
                objectName: manageObjectArgs.objectName,
                label: manageObjectArgs.label,
                pluralLabel: manageObjectArgs.pluralLabel,
                description: manageObjectArgs.description,
                nameFieldLabel: manageObjectArgs.nameFieldLabel,
                nameFieldType: manageObjectArgs.nameFieldType,
                sharingModel: manageObjectArgs.sharingModel
            };
            return await handleManageObject(conn, validatedArgs);
        }
        case "salesforce_manage_field": {
            const manageFieldArgs = args;
            ...
            const validatedArgs = {
                operation: manageFieldArgs.operation,
                objectName: manageFieldArgs.objectName,
                fieldName: manageFieldArgs.fieldName,
                label: manageFieldArgs.label,
                type: manageFieldArgs.type,
                required: manageFieldArgs.required,
                unique: manageFieldArgs.unique,
                externalId: manageFieldArgs.externalId,
                length: manageFieldArgs.length,
                precision: manageFieldArgs.precision,
                scale: manageFieldArgs.scale,
                referenceTo: manageFieldArgs.referenceTo
            };
            return await handleManageField(conn, validatedArgs);
        }
        case "salesforce_search_all": {
            const searchAllArgs = args;
            if (!searchAllArgs.searchTerm) {
                throw new Error('searchTerm is required for search all');
            }
            const validatedArgs = {
                searchTerm: searchAllArgs.searchTerm,
                objects: (searchAllArgs.objects || []),
                searchIn: searchAllArgs.searchIn,
                withClauses: searchAllArgs.withClauses
            };
            return await handleSearchAll(conn, validatedArgs);
        }
        case "salesforce_read_apex": {
            const readApexArgs = args;
            if (!readApexArgs.className) {
                throw new Error('className is required for read apex');
            }
            const validatedArgs = {
                className: readApexArgs.className
            };
            return await handleReadApex(conn, validatedArgs);
        }
        case "salesforce_write_apex": {
            const writeApexArgs = args;
        ...
        case "salesforce_write_apex_trigger": {
            ...
            const validatedArgs = {
                operation: writeTriggerArgs.operation,
                triggerName: writeTriggerArgs.triggerName,
                body: writeTriggerArgs.body,
                objectName: writeTriggerArgs.objectName,
                apiVersion: writeTriggerArgs.apiVersion
            };
        ...
        case "salesforce_manage_debug_logs": {
            ...
            const validatedArgs = {
                operation: debugArgs.operation,
                username: debugArgs.username,
                logLevel: debugArgs.logLevel,
                expirationTime: debugArgs.expirationTime
            };
