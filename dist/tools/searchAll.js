...
function buildWithClause(withClause) {
    ...
    try {
      ...
      ${accessClause}`.trim();
        ...
        objects.forEach((obj, index) => {
            const objectResults = result.searchRecords.filter((record) => record.attributes.type === obj.name);
            formattedResults += `\n${obj.name} (${objectResults.length} records found):\n`;
            if (objectResults.length > 0) {
                objectResults.forEach((record, recordIndex) => {
                    ...
                    if (withClauses?.some(w => w.type === "SNIPPET")) {
                        formattedResults += `    Snippets:\n${record.snippets?.map((s) => `      ${s.field}: ${s.snippet}`).join('\n') || '      None'}\n`;
                    }
                });
            }
