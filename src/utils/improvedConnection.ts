...
import { persistentTokenStore, UserConnection } from './persistentTokenStore.js';
 ...
 */
export async function createUserSalesforceConnection(userId: string): Promise<ConnectionResult> {
  const userConnection = await persistentTokenStore.getConnectionByUserId(userId);
  
  if (!userConnection) {
    throw new SalesforceConnectionError(
      `No Salesforce connection found for user: ${userId}`,
      'NO_CONNECTION',
      401
    );
  }
  ...
  try {
    ...
    conn.on('refresh', async (accessToken: string, res: any) => {
    ...
    if (error instanceof Error && error.message.includes('Session expired')) {
      ...
      throw new SalesforceConnectionError(
        'Session expired and refresh failed. Please re-authenticate.',
        'SESSION_EXPIRED',
        401
      );
    }
    ...
    throw new SalesforceConnectionError(
      `Failed to create connection for user ${userId}`,
      'CONNECTION_FAILED',
      500,
      true
    );
 ...
 */
export async function createSessionSalesforceConnection(sessionId: string): Promise<ConnectionResult> {
  const userConnection = await persistentTokenStore.getConnectionBySession(sessionId);
  
  if (!userConnection) {
    throw new SalesforceConnectionError(
      `No Salesforce connection found for session: ${sessionId}`,
      'NO_SESSION',
      401
    );
  }
      ...
      res.on('end', () => {
        try {
          ...
          if (res.statusCode !== 200) {
            const error = new SalesforceConnectionError(
              `OAuth token request failed: ${parsedData.error} - ${parsedData.error_description}`,
          ...
          reject(new SalesforceConnectionError(
            `Failed to parse OAuth response: ${e instanceof Error ? e.message : String(e)}`,
            'PARSE_ERROR',
            500
          ));
        }
...
async function refreshUserToken(userConnection: UserConnection): Promise<{ accessToken: string } | null> {
  try {
