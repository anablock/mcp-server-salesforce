...
declare global {
  namespace Express {
    interface Request {
      salesforceConnection?: any; // Will be jsforce connection
      userId?: string;
    }
  }
}
...
export interface TokenRefreshMiddlewareOptions {
  ...
  refreshThresholdMinutes?: number; // Refresh tokens when they expire within this threshold
}

export function createTokenRefreshMiddleware(options: TokenRefreshMiddlewareOptions) {
...
function shouldRefreshToken(tokens: any, thresholdMinutes: number): boolean {
  if (!tokens.expiresAt) {
...
async function refreshTokens(oauth: SalesforceOAuth, connection: any): Promise<boolean> {
  try {
    const tokenData = await oauth.refreshToken(
      connection.tokens.refreshToken,
      connection.tokens.instanceUrl
    );
...
export function addAuthHeaders(req: Request, res: Response, next: NextFunction) {
